    ...
    methods: {
        getData:function(){
            ...
            //    .then(response => response.json()).then((data) =>{ this.list = data;console.log("data",this.list); this.subscribeNo= this.list.items[0].statistics.subscriberCount; }
               //)
               fetch('https://www.googleapis.com/youtube/v3/channels/?part=snippet,contentDetails,statistics&id=UCaJZ13Vz8f5p4cPWLrUCfrg&key=AIzaSyCC9GCpSlFnw1jFu8jmiW4KiaZ-828OK_k')
                .then(response => response.json()).then((data) => this.channelDetails = data).catch(err => console.log(err));
                fetch('https://www.googleapis.com/youtube/v3/playlists?part=snippet,contentDetails&channelId=UCq-Fj5jknLsUf-MWSy4_brA&maxResults=10&key=AIzaSyCC9GCpSlFnw1jFu8jmiW4KiaZ-828OK_k')
